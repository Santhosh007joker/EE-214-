...
entity UART is
  generic (                           // Generic types can be set from outside and default value is 521 though, not a signal so does not change while running
    g_CLKS_PER_BIT : integer := 521   -- For 5 MHz clock and 9600 baud // (1/9600) / (1/5,000,000) = 521
  );
  port (
...
architecture rtl of UART is
  ...
  type t_SM_Main is (s_Idle, s_RX_Start_Bit, s_RX_Data_Bits,
                     s_RX_Stop_Bit, s_Cleanup);        // A signal declared like this can only tke one of the given (5) values
  ...
  signal r_RX_Byte   : std_logic_vector(7 downto 0) := (others => '0');   //  others => '0' is shorthand hand for the 10,000K
...
begin
  ...
  u_clk_div : ClockDiv       // similar to making the g1 : AND_2 port map () stuff
    port map (
      clk_in       => i_Clk ,
      reset        => '0'   ,            //count = 50 MHz /(2*f) = 5 for 5MHz
      clk_out_5MHz =>  
      clk_out_2Hz  => ----
    );
